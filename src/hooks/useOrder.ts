...
export const useOrder = ({ addToast }: UseOrderProps) => {
  ...
  const [pendingItemForNameAssignment, setPendingItemForNameAssignment] = useState<{
  ...
  const customersOnBill = useMemo(
    () => [...new Set(cartItems.map((item) => item.customerName))].sort(),
    [cartItems]
  );
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCartItems((prevItems) => {
        const needsUpdate = prevItems.some(
          (item) => item.status === 'pending' && Date.now() - item.addedAt > 20000
        );
        if (!needsUpdate) return prevItems;
        return prevItems.map((item) =>
          item.status === 'pending' && Date.now() - item.addedAt > 20000
            ? { ...item, status: 'ordered' }
            : item
        );
      });
  ...
  const handleReorder = useCallback(
    (order: Order) => {
      ...
      order.items.forEach((item) => {
        ...
        if (fullItem)
          handleAddToCart(fullItem, customerForReorder, item.quantity, item.customizations);
      });
    },
  ...
  const handleReassignCartItem = (cartItemId: string, newCustomerName: string) => {
    setCartItems((prev) =>
      prev.map((item) =>
        item.cartItemId === cartItemId ? { ...item, customerName: newCustomerName } : item
      )
    );
    setCartUpdateIndicator((c) => c + 1);
  };

  const handleAddNewItemToCustomer = (
    itemType: 'menu' | 'drink',
    itemId: number,
    ...
    const itemToAdd =
      itemType === 'menu'
        ? MENU_ITEMS.find((i) => i.id === itemId)
        : [...DRINK_ITEMS, ...MORE_DRINK_ITEMS].find((i) => i.id === itemId);
    if (itemToAdd) {
      if (customerName) handleAddToCart(itemToAdd, customerName, 1, { removed: [], added: [] });
      else
        handleInitiateAddToCart(itemToAdd, 1, { removed: [], added: [] }, () => {
          console.log('Prompting for name for new diner');
        });
    }
  };
  ...
  const handleReadBillAloud = async () => {
    ...
    try {
      const itemsByCustomer = cartItems.reduce(
        (acc, item) => {
  ...
  return {
    ...
    orderHistory,
    isGeneratingAudio,
    pendingItemForNameAssignment,
    setPendingItemForNameAssignment,
    handleAddToCart,
