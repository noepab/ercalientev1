...
export const MoreDrinksModal: React.FC<MoreDrinksModalProps> = ({ isOpen, onClose, onReviewAndAdd, drinks, onEditImage }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-3xl font-bold mb-6 text-center font-heading">Carta de Bebidas Completa</h2>
        <div className="flex-grow overflow-y-auto -mx-4 px-4">
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-x-4 gap-y-8">
                {drinks.map(drink => (
                    <DrinkIcon key={`${drink.id}-${drink.name}`} drink={drink} onReviewAndAdd={onReviewAndAdd} isReadyToOrder={true} onEditImage={onEditImage} />
                ))}
            </div>
        </div>
      </motion.div>
