  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 0,
      sender: 'ai',
      text: '¡Hola! Soy tu asistente virtual. ¿En qué puedo ayudarte? Puedo buscar información en la web o encontrar lugares cercanos.',
    },
  ]);
          ...
          <AnimatePresence>
            {messages.map((msg) => (
              ...
              >
                ...
                <div
                  className={`max-w-xs md:max-w-md p-3 rounded-lg ${msg.sender === 'user' ? 'bg-blue-800' : msg.sender === 'error' ? 'bg-red-800' : 'bg-slate-600'}`}
                >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {msg.sources.map((source, index) => (
                          <a
                            ...
                            key={index}
                            className="text-xs bg-slate-700 hover:bg-slate-500 rounded-full px-2 py-1 flex items-center gap-1 transition-colors"
                          >
        ...
        <div className="p-4 border-t border-[var(--color-surface-light)]">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-3 bg-blue-800 rounded-full text-white hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed transition-colors"
            >
