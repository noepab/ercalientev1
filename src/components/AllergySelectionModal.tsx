    ...
    >
      ...
      >
        ...
        <p className="text-slate-400 mb-6">
          Selecciona los alérgenos que quieres excluir del menú. Los platos que los contengan se
          ocultarán.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {ALLERGIES.map((allergy) => (
            <label
              key={allergy}
              className="flex items-center space-x-3 p-3 bg-[var(--color-surface-light)] rounded-lg cursor-pointer hover:bg-slate-600"
            >
              <input
                type="checkbox"
        ...
        <div className="mt-8 text-center">
          <button
            onClick={onClose}
            className="bg-[var(--color-primary)] text-[var(--color-text-dark)] py-2 px-6 rounded-lg font-bold hover:bg-[var(--color-primary-dark)] transition-colors"
          >
            Hecho
          </button>
        </div>
