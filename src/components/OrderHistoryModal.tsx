...
const OrderCard: React.FC<{ order: Order; onReorder: (order: Order) => void }> = ({
  order,
  onReorder,
}) => (
  <div className="bg-[var(--color-surface-light)] p-4 rounded-lg">
    <div className="flex justify-between items-center border-b border-slate-600 pb-2 mb-2">
      ...
      <div className="text-right">
        ...
        <p className="text-xs text-slate-400">
          {order.diningOption === 'dine-in' ? `Mesa ${order.tableNumber}` : 'Para llevar'}
        </p>
      </div>
    </div>
    <div className="space-y-1 mb-3">
      ...
      {order.items.length > 3 && (
        <p className="text-xs text-slate-500 text-center">... y {order.items.length - 3} más</p>
      )}
    </div>
    <button
      onClick={() => onReorder(order)}
      className="w-full bg-indigo-700 hover:bg-indigo-600 text-white text-sm font-semibold py-1.5 rounded-md transition-colors"
    >
      Volver a Pedir
    </button>
  </div>
