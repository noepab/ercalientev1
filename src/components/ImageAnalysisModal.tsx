  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      const matchedItem = menuItems.find(
        (item) => item.name.toLowerCase() === matchedItemName.toLowerCase()
      );
    ...
    >
      ...
      >
        ...
        <p className="text-center text-slate-400 mb-6">
          Sube una foto de un plato y te diremos si lo tenemos en la carta.
        </p>
        ...
        {image && (
          <button
            ...
            disabled={isLoading}
            className="w-full mt-4 bg-purple-800 text-white py-2 rounded-lg font-bold hover:bg-purple-700 transition-colors disabled:bg-slate-600"
          >
            {' '}
            {isLoading ? 'Analizando...' : 'Buscar en el menú'}{' '}
          </button>
        )}
        ...
        {result && (
          <div className="mt-6 p-4 bg-[var(--color-surface-light)] rounded-lg flex items-center gap-4">
            ...
            <button
              ...
              className="bg-green-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-green-500"
            >
              Añadir
            </button>
          </div>
