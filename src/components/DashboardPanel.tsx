...
export const DashboardPanel: React.FC<DashboardPanelProps> = ({ isOpen, onClose, orders }) => {
  const stats = useMemo(() => {
    ...
    const mostPopularItem = allItems.reduce(
      (acc, item) => {
    ...
    const revenueByDiningOption = orders.reduce(
      (acc, order) => {
  ...
  const totalRevenueForChart =
    (stats.revenueByDiningOption['dine-in'] || 0) + (stats.revenueByDiningOption['takeout'] || 0);
          ...
          >
            ...
            <div className="flex-grow flex">
              ...
              <div className="w-48 bg-slate-800/50 p-4 flex flex-col space-y-2">
                <a
                  href="#"
                  className="flex items-center gap-3 px-3 py-2 text-white bg-slate-700 rounded-md"
                >
                  <LayoutDashboardIcon className="w-5 h-5" /> Visión General
                </a>
                <a
                  href="#"
                  className="flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-700 rounded-md"
                >
                  <UserIcon className="w-5 h-5" /> Usuarios
                </a>
                <a
                  href="#"
                  className="flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-700 rounded-md"
                >
                  <UtensilsIcon className="w-5 h-5" /> Menú
                </a>
                <a
                  href="#"
                  className="flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-700 rounded-md"
                >
                  <BarChartIcon className="w-5 h-5" /> Analíticas
                </a>
                <a
                  href="#"
                  className="flex items-center gap-3 px-3 py-2 text-slate-300 hover:bg-slate-700 rounded-md mt-auto"
                >
                  ...
                  <div className="space-y-2">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-white font-medium">
                          ${(stats.revenueByDiningOption['dine-in'] || 0).toFixed(2)}
                        </span>
                      </div>
                      <div className="w-full bg-slate-600 rounded-full h-2.5">
                        <div
                          ...
                          style={{
                            width: `${totalRevenueForChart > 0 ? ((stats.revenueByDiningOption['dine-in'] || 0) / totalRevenueForChart) * 100 : 0}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-white font-medium">
                          ${(stats.revenueByDiningOption['takeout'] || 0).toFixed(2)}
                        </span>
                      </div>
                      <div className="w-full bg-slate-600 rounded-full h-2.5">
                        <div
                          ...
                          style={{
                            width: `${totalRevenueForChart > 0 ? ((stats.revenueByDiningOption['takeout'] || 0) / totalRevenueForChart) * 100 : 0}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    ...
                    {orders.slice(0, 5).map((order) => (
                      <div key={order.id} className="bg-slate-700/50 p-3 rounded-md text-sm">
                        ...
                        <p className="text-xs text-slate-400">
                          {order.items.length} artículos &bull;{' '}
