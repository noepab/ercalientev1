...
export const RecommendationModal: React.FC<RecommendationModalProps> = ({ isOpen, onClose, onReviewAndAdd, diningOption }) => {
  ...
  const getRecommendations = async () => {
    ...
    try {
      ...
      if(isExpertMode && expertPrompt) {
        const menuString = [...MENU_ITEMS, ...DRINK_ITEMS, ...MORE_DRINK_ITEMS].map(item => `${item.name} - ${'description' in item ? item.description : 'Bebida'}`).join('\n');
        ...
        const response = await ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: {
              ...
              items: {
                  type: Type.OBJECT,
                  properties: { id: { type: Type.NUMBER }, name: { type: Type.STRING }, price: { type: Type.NUMBER }, imageUrl: { type: Type.STRING } }
              }
            }
          }
        });
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-3xl font-bold mb-4 text-center font-heading">Recomendaciones de la IA</h2>
        
        <div className="flex justify-center mb-6">
            <label className="flex items-center cursor-pointer">
                <span className={`mr-3 font-semibold ${!isExpertMode ? 'text-[var(--color-primary)]' : 'text-slate-400'}`}>Sugerencias Rápidas</span>
                <div className="relative">
                    <input type="checkbox" checked={isExpertMode} onChange={() => setIsExpertMode(!isExpertMode)} className="sr-only" />
                    <div className="block bg-slate-600 w-14 h-8 rounded-full"></div>
                    <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${isExpertMode ? 'transform translate-x-6 bg-purple-400' : ''}`}></div>
                </div>
                <span className={`ml-3 font-semibold ${isExpertMode ? 'text-purple-400' : 'text-slate-400'}`}>Modo Experto</span>
                <BrainCircuitIcon className={`w-5 h-5 ml-2 ${isExpertMode ? 'text-purple-400' : 'text-slate-400'}`} />
            </label>
        </div>
        ...
        {recommendations.length === 0 && !expertResponse && !isLoading && !error && (
            <div className="text-center py-8">
                {isExpertMode ? (
                    <>
                        <p className="text-slate-400 mb-4">Describe tu petición para una recomendación detallada. <br/> (Ej: "Busco algo ligero y sin gluten para cenar")</p>
                        <textarea value={expertPrompt} onChange={(e) => setExpertPrompt(e.target.value)} rows={3} className="w-full max-w-md mx-auto p-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="¿Qué te apetece?"/>
                    </>
                ) : (
                    <p className="text-slate-400 mb-6">¿No sabes qué pedir? Deja que la IA te sugiera algo delicioso.</p>
                )}
                <button onClick={getRecommendations} disabled={isExpertMode && !expertPrompt} className="bg-sky-800 hover:bg-sky-700 text-white font-bold py-3 px-6 rounded-lg transition-colors flex items-center gap-2 mx-auto mt-4 disabled:bg-slate-600 disabled:cursor-not-allowed">
        ...
        {recommendations.length > 0 && (
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                {recommendations.map(item => (
                    <div key={item.id} className="bg-[var(--color-surface-light)] rounded-lg p-4 text-center">
                        {item.imageUrl ? (
                        ...
                        <button onClick={() => handleAddToCartClick(item)} className="mt-2 w-full bg-[var(--color-primary)] text-sm text-[var(--color-text-dark)] py-1.5 rounded-md font-semibold hover:bg-[var(--color-primary-dark)] transition-colors">Revisar/Añadir</button>
                    </div>
                ))}
            </div>
        )}
        ...
        {expertResponse && (
            <div className="mt-6 p-4 bg-slate-800/50 rounded-lg max-h-80 overflow-y-auto">
                ...
                <div className="prose prose-invert prose-sm text-slate-300 whitespace-pre-wrap">{expertResponse}</div>
            </div>
        )}
