  ...
  const addToCartFunctionDeclaration: FunctionDeclaration = useMemo(
    () => ({
      ...
      parameters: {
        type: Type.OBJECT,
        description:
          'Añade uno o más artículos a la comanda del cliente. Utiliza esto cuando el cliente pida comida o bebida.',
        properties: {
          items: {
            type: Type.ARRAY,
            ...
            items: {
              ...
              properties: {
                ...
                quantity: {
                  type: Type.NUMBER,
                  description: 'La cantidad del artículo. Por defecto 1.',
                },
              },
          ...
          customerName: {
            type: Type.STRING,
            description:
              'El nombre del cliente para quien es el pedido. Si no se especifica, se preguntará o asignará al cliente por defecto.',
          },
        },
        required: ['items'],
      },
            ...
            if (typeof base64Audio === 'string') {
              ...
              const decodeAudioData = async (
                ...
                for (let i = 0; i < frameCount; i++)
                  channelData[i] = dataInt16[i * 1 + 0] / 32768.0;
                return buffer;
              };
            ...
            if (message.toolCall) {
              for (const fc of message.toolCall.functionCalls) {
                if (fc.name === 'addToCart') {
                  const { items, customerName } = fc.args as {
                    items?: { itemId: number; quantity: number }[];
                    customerName?: string;
                  };
                  let resultMessage = '';
                  if (items && Array.isArray(items)) {
                    const finalCustomerName = customerName || customersOnBill[0];
