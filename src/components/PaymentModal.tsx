            ...
            <motion.div key="form" exit={{ opacity: 0, transition: { duration: 0.1 } }}>
              ...
              <div className="mb-6">
                ...
                <div className="flex justify-center space-x-2 mb-3">
                  {tipPercentages.map((p) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg transition-colors ${tipPercentage === p ? 'bg-[var(--color-primary)] text-[var(--color-text-dark)] font-bold' : 'bg-[var(--color-surface-light)] hover:bg-slate-600'}`}
                    >
                      {p}%
                    </button>
                  ))}
                ...
                <input
                  ...
                  className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
                />
              </div>
              <p className="text-xs text-slate-500 mb-6 text-center">
                (Esto es una simulación. No se realizará ninguna transacción real.)
              </p>
              <button
                onClick={handlePayment}
                disabled={paymentStatus === 'processing'}
                className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-500 transition-colors disabled:bg-slate-600 disabled:cursor-wait"
              >
