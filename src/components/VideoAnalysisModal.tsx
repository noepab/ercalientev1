    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        >
            ...
            {preview ? <video src={preview} controls className="w-full h-full object-contain rounded-lg" /> : 
            <div>
        ...
        <button onClick={analyzeVideo} disabled={isLoading || !videoFile || !prompt} className="w-full mt-4 bg-purple-800 text-white py-2 rounded-lg font-bold hover:bg-purple-700 transition-colors disabled:bg-slate-600 disabled:cursor-not-allowed flex items-center justify-center gap-2">
             {isLoading ? 'Analizando...' : <><ClapperboardIcon className="w-5 h-5"/> Analizar Vídeo</>}
        </button>
