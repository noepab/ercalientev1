...
export const CustomizationModal: React.FC<CustomizationModalProps> = ({ isOpen, onClose, onConfirm, item }) => {
  ...
  React.useEffect(() => {
    if (!item) return;
    const extrasPrice = item.extras?.filter(e => selectedExtras.has(e.name)).reduce((sum, e) => sum + e.price, 0) || 0;
  ...
  const handleConfirm = () => {
    const customizations: CartItem['customizations'] = {
      removed: item.ingredients?.filter(i => i.default && !selectedIngredients.has(i.name)).map(i => i.name) || [],
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex-shrink-0">
          ...
          {item.imageUrl ? <img src={item.imageUrl} alt={item.name} className="w-full h-40 object-cover rounded-lg mb-4" /> : <div className="w-full h-40 bg-slate-700 rounded-lg mb-4 flex items-center justify-center"><ImageIcon className="w-16 h-16 text-slate-500" /></div>}
          <h2 className="text-2xl font-bold font-heading">{item.name}</h2>
          <p className="text-lg text-slate-400 font-bold mb-4">Precio base: ${item.price.toFixed(2)}</p>
        </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {item.ingredients.map(ing => (
                  <label key={ing.name} className="flex items-center justify-between p-2 bg-[var(--color-surface-light)] rounded-md cursor-pointer"><span className="text-slate-200">{ing.name}</span><input type="checkbox" checked={selectedIngredients.has(ing.name)} onChange={() => handleIngredientToggle(ing.name)} className="h-5 w-5 rounded border-gray-300 text-amber-500 focus:ring-amber-500" /></label>
                ))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="space-y-2">
                {item.extras.map(extra => (
                  <label key={extra.name} className="flex items-center justify-between p-2 bg-[var(--color-surface-light)] rounded-md cursor-pointer"><span className="text-slate-200">{extra.name} <span className="text-xs text-green-400">(+${extra.price.toFixed(2)})</span></span><input type="checkbox" checked={selectedExtras.has(extra.name)} onChange={() => handleExtraToggle(extra.name)} className="h-5 w-5 rounded border-gray-300 text-amber-500 focus:ring-amber-500" /></label>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="flex-shrink-0 pt-4">
          <div className="p-3 bg-slate-800/50 rounded-lg mb-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-purple-300"> <BrainCircuitIcon className="w-5 h-5"/> Análisis de Alérgenos IA </div>
              <div className="text-slate-300 text-sm mt-1 h-5">
                  {isAnalyzing ? 'Analizando...' : allergenInfo || 'Selecciona ingredientes para analizar.'}
              </div>
          </div>
          <div className="flex items-center justify-center space-x-4 mb-4">
            <button onClick={() => handleQuantityChange(-1)} className="w-10 h-10 bg-[var(--color-surface-light)] rounded-full font-bold text-xl flex-shrink-0">-</button>
            <span className="text-2xl font-bold w-12 text-center">{quantity}</span>
            <button onClick={() => handleQuantityChange(1)} className="w-10 h-10 bg-[var(--color-surface-light)] rounded-full font-bold text-xl flex-shrink-0">+</button>
          </div>
          <button onClick={handleConfirm} className="w-full bg-[var(--color-primary)] text-[var(--color-text-dark)] py-3 rounded-lg font-bold hover:bg-[var(--color-primary-dark)] transition-colors">Añadir {quantity} por ${finalPrice.toFixed(2)}</button>
        </div>
