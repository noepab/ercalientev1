...
import { XIcon, Wand2Icon, ChevronLeftIcon, ChevronRightIcon, ImageIcon, DownloadIcon, UploadCloudIcon } from './icons';
...
export const ImageGenerationModal: React.FC<ImageGenerationModalProps> = ({ isOpen, onClose, onUpdateImage, itemToEdit, onNavigateNext, onNavigatePrev }) => {
...
Descripción: Una fotografía de producto de un plato de "${itemToEdit.name}", con las siguientes características: ${prompt}. La imagen debe ser sobre un fondo blanco liso y aislado. El estilo debe ser de restaurante, apetitoso, de alta calidad y optimizado para la creación de modelos 3D.`;
            ...
            if (!imageFound) {
                ...
                throw new Error("No se pudo generar una imagen con esa descripción. Por favor, intenta ser más descriptivo sobre la apariencia visual del plato.");
            }
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors z-30"> <XIcon className="w-6 h-6" /> </button>
                <h2 className="text-3xl font-bold mb-2 text-center font-heading">Estudio de Imagen</h2>
                <p className="text-slate-400 mb-6 text-center">Genera una imagen con IA o sube un archivo.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    <div className="space-y-4 relative">
                        <div className="aspect-square bg-slate-700 rounded-lg overflow-hidden flex items-center justify-center relative">
                            <button onClick={onNavigatePrev} className="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-black/50 p-2 rounded-full text-white hover:bg-black/80 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
                                <ChevronLeftIcon className="w-6 h-6" />
                            </button>
                            <button onClick={onNavigateNext} className="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-black/50 p-2 rounded-full text-white hover:bg-black/80 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
                                <ChevronRightIcon className="w-6 h-6" />
                            </button>
                            {generatedImage ? 
                                <img src={generatedImage} className="w-full h-full object-cover" alt="Generated" /> : 
                                itemToEdit && itemToEdit.imageUrl ? (
                                    <img src={itemToEdit.imageUrl} className="w-full h-full object-cover" alt={itemToEdit.name} />
                                ) : (
                        ...
                        <h3 className="text-lg font-semibold text-center">{itemToEdit?.name || 'Crea una imagen'}</h3>
                    </div>
                    <div className="space-y-4 flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-2">
                           <span className="text-xs text-slate-400 self-center">Sugerencias:</span>
                            {examplePrompts.map(p => (
                                <button key={p} onClick={() => setPrompt(p)} className="text-xs bg-slate-600 hover:bg-slate-500 px-2 py-1 rounded-full transition-colors" disabled={!itemToEdit}>{p}</button>
                            ))}
                        </div>
                        ...
                        <div className="space-y-2">
                            <button onClick={generateImage} disabled={isLoading || !prompt || !itemToEdit} className="w-full bg-pink-800 text-white py-2 rounded-lg font-bold hover:bg-pink-700 transition-colors disabled:bg-slate-600 disabled:cursor-wait flex items-center justify-center gap-2">
                                {isLoading ? 'Procesando...' : <><Wand2Icon className="w-5 h-5"/> Generar con IA</>}
                            </button>
                            ...
                            <button onClick={() => fileInputRef.current?.click()} disabled={isLoading || !itemToEdit} className="w-full bg-slate-600 text-white py-2 rounded-lg font-bold hover:bg-slate-500 transition-colors flex items-center justify-center gap-2">
                            ...
                            {generatedImage && (
                                <div className="grid grid-cols-2 gap-2 pt-2">
                                    <button onClick={handleDownload} className="w-full bg-sky-700 text-white py-2 rounded-lg font-bold hover:bg-sky-600 transition-colors flex items-center justify-center gap-2">
                                        <DownloadIcon className="w-5 h-5"/> Descargar
                                    </button>
                                    <button onClick={handleConfirm} className="w-full bg-green-600 text-white py-2 rounded-lg font-bold hover:bg-green-500 transition-colors">Confirmar y Guardar</button>
                                </div>
                            )}
                        </div>
                    </div>
