...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, onReviewAndAdd, isReadyToOrder, onEditImage, onGenerateModel }) => {
        ...
        <>
            ...
            >
                <div className="relative w-full h-56 bg-slate-700">
                    {item.imageUrl ? (
                        <img loading="lazy" src={item.imageUrl} alt={`Imagen de ${item.name}`} className="w-full h-full object-cover pointer-events-none" />
                    ) : (
                    ...
                    <div className="absolute top-2 right-2 flex flex-col gap-2">
                        <button onClick={() => onGenerateModel(item)} className="bg-slate-900/50 text-white p-2 rounded-full backdrop-blur-sm hover:bg-slate-900/75 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 focus-visible:ring-[var(--color-primary)]" aria-label="Ver o generar modelo 3D">
                ...
                <div className="p-5 flex flex-col flex-grow">
                    <div className="flex justify-between items-start">
                        ...
                        <span className="text-xs flex-shrink-0 font-semibold bg-slate-700 text-[var(--color-primary)] px-2 py-1 rounded-full">{item.category}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-4">
                        <span className="text-2xl font-bold text-[var(--color-primary)]">${item.price.toFixed(2)}</span>
                        <motion.button
                            whileTap={{ scale: 0.95 }}
