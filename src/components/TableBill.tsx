...
const TableBillComponent: React.FC<TableBillProps> = ({
  items,
  tableNumber,
  onPay,
  onClear,
  ...
  const diningTitle =
    diningOption === 'dine-in' ? `Mesa ${tableNumber || '...'}` : 'Pedido para Llevar';
  ...
  return (
    <div
      className={`bg-[var(--color-surface)] rounded-xl shadow-lg border border-[var(--color-surface-light)] transition-all duration-300 ${isGlowing ? 'shadow-amber-500/30 border-amber-500/50 animate-heartbeat' : ''} ${dragOverTarget ? 'ring-2 ring-[var(--color-primary)]/80 ring-offset-2 ring-offset-[var(--color-bg)]' : ''}`}
    >
      <div className="p-5 border-b border-[var(--color-surface-light)] flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onReadAloud}
            disabled={isGeneratingAudio || items.length === 0}
            className="text-slate-400 hover:text-[var(--color-primary)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            aria-label="Leer la cuenta en voz alta"
          >
            {isGeneratingAudio ? (
          ...
          {diningOption === 'dine-in' && tableNumber && (
            <button
              ...
              aria-label="Mostrar código QR para unirse a la mesa"
            >
      ...
      <div className="p-5">
        {items.length === 0 ? (
          <div
            className={`text-center py-10 transition-all border-2 border-dashed rounded-lg ${dragOverTarget ? 'border-[var(--color-primary)] bg-slate-700/50 scale-105' : 'border-transparent'}`}
            onDragOver={(e) => {
              e.preventDefault();
              setDragOverTarget('__NEW_DINER__');
            }}
            onDragLeave={() => setDragOverTarget(null)}
            onDrop={(e) => handleDrop(e, null)}
          >
            <div
              className={`w-16 h-16 bg-slate-700/50 mx-auto rounded-full flex items-center justify-center mb-4 transition-transform ${dragOverTarget ? 'scale-110' : ''}`}
            >
              {dragOverTarget ? (
                <CheckCircleIcon className="w-8 h-8 text-[var(--color-primary)]" />
              ) : (
                <MicIcon className="w-8 h-8 text-slate-400" />
              )}
            </div>
            <p className="font-semibold text-slate-300">
              {dragOverTarget ? '¡Suelta para añadir!' : 'Tu comanda está vacía'}
            </p>
            <p className="text-slate-400 text-sm">
              {dragOverTarget
                ? 'Se te pedirá un nombre.'
                : '¡Pulsa el micrófono o arrastra un plato aquí!'}
            </p>
          </div>
        ) : (
              ...
              {sortedCustomerNames.map((customerName) => (
                <motion.div
                  ...
                  onDragOver={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    setDragOverTarget(customerName);
                  }}
                  onDragLeave={() => setDragOverTarget(null)}
                  onDrop={(e) => handleDrop(e, customerName)}
                >
                  <h3 className="font-semibold text-[var(--color-primary)] text-xl mb-1 px-2 font-heading">
                    {customerName}
                  </h3>
                  {itemsByCustomer[customerName].map((item) => (
                    <CartItemRow
                      ...
                      customersOnBill={customersOnBill}
                    />
                  ))}
                </motion.div>
              ))}
      ...
      <div className="p-5 border-t border-[var(--color-surface-light)] space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            ...
            className="w-full bg-slate-600 text-white py-3 rounded-lg font-bold hover:bg-slate-500 transition-colors disabled:bg-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed"
          >
            Limpiar
          </button>
          <button
            onClick={onPay}
            disabled={items.length === 0}
            className="w-full bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-500 transition-colors disabled:bg-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed"
          >
            Pagar
          </button>
        </div>
