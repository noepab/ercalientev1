...
export const ModelGenerationModal: React.FC<ModelGenerationModalProps> = ({ isOpen, onClose, item, onUpdateModel }) => {
  ...
  const handleGenerateModel = async () => {
    ...
    setTimeout(() => {
        ...
        const exampleModelUrl = item.id === 1 
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-3xl font-bold mb-4 text-center font-heading">Modelo 3D de {item.name}</h2>
          ...
          {item.modelUrl ? (
            ...
            <motion.div
              ...
              >
                ...
                <p className="text-slate-400 mb-4">Describe el objeto que quieres crear. Sé lo más detallado posible para un mejor resultado.</p>
                <textarea 
                    value={prompt}
