...
export const DrinkIcon: React.FC<DrinkIconProps> = ({ drink, onReviewAndAdd, isReadyToOrder, onEditImage }) => {
    ...
    const allergenDisplay = (() => {
        ...
        if (drink.allergens.length === 0) return <p className="text-[10px] text-green-400 leading-tight mt-0.5">Sin alérgenos</p>;
            ...
            <motion.div whileTap={{ scale: 0.9 }}>
                ...
                >
                    <div className="w-20 h-20 bg-[var(--color-surface)] rounded-full flex items-center justify-center overflow-hidden transform group-hover:scale-110 transition-transform border-2 border-[var(--color-surface-light)] group-hover:border-[var(--color-primary)] pointer-events-none relative">
                        {drink.imageUrl ? (
                            <img loading="lazy" src={drink.imageUrl} alt={`Imagen de ${drink.name}`} className="w-full h-full object-cover" />
                        ) : (
